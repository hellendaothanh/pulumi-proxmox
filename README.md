...
### 1. 🛡️ Quản Trị Tài Nguyên & Phân Quyền (Governance & Multi-tenancy)
* **Phân Quyền Người Dùng (Role-Based Access Control - RBAC)**:
  * **👑 Administrator**: Toàn quyền quản trị hạ tầng; được phép tạo, cấu hình, thay đổi, xóa máy ảo và snapshots trên mọi môi trường (`DEV`, `STAGING`, `PROD`).
  * **👨‍💻 Developer**: Bị giới hạn nghiêm ngặt theo chính sách Multi-tenancy — **chỉ được phép khởi tạo và xóa máy ảo trên môi trường `DEV`**; hệ thống tự động khóa và từ chối mọi thao tác trên môi trường `STAGING` và `PROD`.
  * **Role Switcher**: Chuyển đổi vai trò linh hoạt ngay trên thanh điều hướng để kiểm thử và phân chia trách nhiệm.
* **Nhật Ký Kiểm Toán (Audit Logs & Compliance)**:
  * Ghi vết đầy đủ mọi hoạt động: **Thời gian thực hiện**, **Danh tính & Vai trò (Role)**, **Hành động (Action)**, **Mục tiêu (Target VM/Stack)**, **Môi trường (Environment)**, **Trạng thái (SUCCESS / DENIED)** và **Chi tiết nguyên nhân**.
  * Chuyên mục **Audit Logs** trực quan giúp truy vết lịch sử triển khai, tuân thủ tiêu chuẩn an toàn thông tin DevSecOps.

### 2. 📊 Giám Sát Tài Nguyên Toàn Cụm & Điều Hướng Thông Minh
...
### 3. ⚡ Quản Lý Vòng Đời VM & Thao Tác Nhanh (VM Lifecycle Management)
...
### 4. 🤖 Tự Động Hóa Sau Khởi Tạo & IaC (Post-provisioning Hooks & Cloud-Init)
...
### 5. 🏗️ Khởi Tạo Máy Ảo Đơn & Cụm Máy Ảo (Single & Multi-Node Batch Deploy)
...
### 6. 🛠️ Bảng Điều Khiển Quản Lý Stack & Terminal Logs (Self-Service Dashboard)
